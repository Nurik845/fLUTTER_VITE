...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _speakIntro() async {
    ...
    // Make LUMI appear happy on intro
    LumiOverlay.set(emotion: LumiEmotion.happy, speech: l.lumiIntro);
    setState(() => speaking = true);
    await TtsService.instance.speak(l.lumiIntro, langCode: _langCode());
  ...
  void _handleStt(String text, bool finalResult) {
    ...
    // Simple intent parsing (RU/KZ/EN)
    if (cmd.contains('СЃРєРѕСЂСѓСЋ') ||
        cmd.contains('103') ||
        cmd.contains('ambulance') ||
        cmd.contains('Р¶РµРґРµР»')) {
      widget.onCommand?.call('emergency_call');
    } else if (cmd.contains('Р°РїС‚РµРє') || cmd.contains('pharmacy')) {
      widget.onCommand?.call('map_pharmacy');
    } else if (cmd.contains('Р±РѕР»СЊРЅРёС†') || cmd.contains('hospital')) {
      widget.onCommand?.call('map_hospital');
    } else if (cmd.contains('РґРёР°РіРЅ') ||
        cmd.contains('diagnos') ||
        cmd.contains('РґРёР°РіРЅРѕР·')) {
  ...
  Future<void> _toggleMic() async {
    ...
    final ok = await SttService.instance.start(
      localeId: _langCode().replaceAll('-', '_'),
      onResult: _handleStt,
    );
    ...
    return Padding(
      ...
      child: ListView(
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            listening ? l.listening : l.notListening,
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [
          Text('Daily tips'),
          SizedBox(height: 8),
          Text('• Пейте воду каждые 2 часа\\n• Дыхание 4–7–8 для спокойствия\\n• Проверьте домашнюю аптечку'),
        ]),
      ),
    );
  }
